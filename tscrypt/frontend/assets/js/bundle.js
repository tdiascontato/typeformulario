...
function hideErrorMessages(form) {
    form.querySelectorAll('.' + show_error).forEach(function (item) { return item.classList.remove(show_error); });
}
...
function shouldSendForm(form) {
    var send = true;
    form.querySelectorAll('.' + show_error).forEach(function () { return (send = false); });
    return send;
}
